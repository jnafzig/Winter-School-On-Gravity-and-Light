...
\begin{figure}[ht]
    \begin{center}
        ...
        \etik
        \caption{Two charts $(U,x)$ and $(V,y)$ used to represent a physical curve $\gamma$. It is assumed that one knows the map $x \circ \gamma$ is so called `undergraduate differentiable'. However, one can not yet conclude whether $y \circ \gamma$ is differentiable, and therefore whether $\gamma$ itself is differentiable, as the continuity of the chart transition map $y \circ x^{-1}$ does not guarantee differentiability (there could be a sudden turning point in the curve representation). }
    \end{center}
\end{figure}
...
\bd[Compatible Charts] 
    Two charts $(U,x)$ and $(V,y)$ of a topological manifold $(\cM,\cO)$ are called \textbf{$\flower$-compatible} if either
    \ben[label=(\alph*)] 
        \item $U\cap V =\emptyset$, or 
        \item $U\cap V \neq \emptyset$ and the chart transition maps $(x\circ y^{-1}):y(U\cap V) \to x(U\cap V)$ and $(y\circ x^{-1}):x(U\cap V) \to y(U\cap V)$ are `undergrad' $\flower$.\footnote{That is they have the property $\flower$ as maps from $\R^d\to \R^d$ that we know from undergraduate courses.}
    \een 
\ed 

\bd[Compatible/Restricted Atlas] 
    An atlas $\cA_{\flower}$ is a \textbf{$\flower$-compatible atlas} if all of its charts are $\flower$-compatible.
\ed 

\bd[]
    A $\flower$-manifold is the triple $(\cM,\cO,\cA_{\flower})$.
\ed 

Now it might be possible that two separate criteria for `tearing our pages' in order to obtain a $\flower$-compatible atlas exist. That is there might be more then one atlas that is $\flower$-compatible. In this case, we have to make a choice about which one to use, but must always remember that we have made this choice, as one of these atlases might allow a different property, say $\flowerf$, to be defined, whereas the other might not. Physically, this is obviously a very important thing to keep in mind, as $\flower$ and $\flowerf$ are physical properties of the curve and therefore don't depend at all on what charts or atlases we choose to use, so we must make sure we pick the ones that match up to the physics.

Before moving on, let's consider the types of things $\flower$ can be. 

\begin{center}
	\begin{tabular}{@{} p{2cm}p{10cm}@{}}
		\toprule
		$\flower$ & Undergraduate $\flower$ \\
		\midrule 
