...
\subsection*{Why the usual definition fails}

Before we do that, though, let us recall how velocity is defined in classical mechanics and see precisely where it breaks down on a manifold. In $\R^n$, if a particle moves along a curve $\gamma:\R\to\R^n$, we define its velocity at time $\lambda_0$ as the limit
\bse
    v := \gamma'(\lambda_0) = \lim_{\ve\to 0} \frac{\gamma(\lambda_0+\ve) - \gamma(\lambda_0)}{\ve}.
\ese
This relies on the vector space structure of $\R^n$: we \textit{subtract} two points $\gamma(\lambda_0+\ve)$ and $\gamma(\lambda_0)$ and divide by a scalar. On a manifold $\cM$, points are just points --- there is no notion of subtraction, and so this definition is simply not available to us.
...
\bq 
    It is clear that both the right-hand side expressions will be elements of $\Hom(C^{\infty}(\cM),\R)$, but we need to check that they are velocities to some curves through $p$. Let's consider them in turn
    \benr 
